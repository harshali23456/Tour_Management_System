...
if (strlen($_SESSION['alogin']) == 0) {
	...
	if (isset($_POST['submit'])) {
		...
		$pdiscount = $_POST['packagediscount'];
		$pfeatures = $_POST['packagefeatures'];
		$pdetails = $_POST['packagedetails'];
		$pimage = $_FILES["packageimage"]["name"];
		$sql = "update TblTourPackages set PackageName=:pname,PackageType=:ptype,PackageLocation=:plocation,PackagePrice=:pprice,PackageFetures=:pfeatures,PackageDetails=:pdetails,Discount=:pdiscount where PackageId=:pid";
		$query = $dbh->prepare($sql);
		$query->bindParam(':pname', $pname, PDO::PARAM_STR);
		$query->bindParam(':ptype', $ptype, PDO::PARAM_STR);
		$query->bindParam(':plocation', $plocation, PDO::PARAM_STR);
		$query->bindParam(':pprice', $pprice, PDO::PARAM_STR);
		$query->bindParam(':pdiscount', $pdiscount, PDO::PARAM_STR);
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<input type="text" class="form-control1" name="packageprice" id="packageprice"
														placeholder=" Package Price is Rupees"
														value="<?php echo htmlentities($result->PackagePrice); ?>" required>
												</div>
											</div>
											<div class="form-group">
												<label for="focusedinput" class="col-sm-2 control-label">Package Discount in
													Percentage</label>
												<div class="col-sm-8">
													<input type="text" class="form-control1" name="packagediscount" id="packageprice"
														placeholder=" Package Discount in Percentage Eg. 10"
														value="<?php echo htmlentities($result->Discount); ?> " % required>
												</div>
											</div>
